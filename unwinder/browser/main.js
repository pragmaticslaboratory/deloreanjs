...
function finishInit(code, breakpoint, container, id) {
  ...
  function removePauseState() {
    if (currentPausedLoc) {
      for (
        var i = currentPausedLoc.start.line;
        i <= currentPausedLoc.end.line;
        i++
      ) {
  ...
  vm.on("paused", function (e) {
    ...
    if (currentPausedLoc) {
      ...
      if (currentPausedLoc.start.line === currentPausedLoc.end.line) {
        ...
        currentExprHighlight = exprHighlight(
          mirror.defaultCharWidth() * width,
  ...
  container
    .querySelector("#run-no-breakpoints")
