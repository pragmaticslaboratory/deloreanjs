  ...
  scopes.forEach(function (scope) {
    if (scope.type !== "global" || asExpr) {
      ...
      scope.references.forEach(function (r) {
        ...
        if (
          defBoxed ||
          (!r.resolved && boxedVars.indexOf(r.identifier.name) !== -1) ||
          (r.resolved &&
...
function visitNode(node, scope, debugInfo) {
  ...
  if (node.expression) {
    ...
    node.body = b.blockStatement([
      withLoc(b.returnStatement(node.body), node.body.loc),
    ]);
  }
  ...
  for (var i = 1, l = em.numTempVars(); i <= l; i++) {
    tmpSave.push(
      b.property("init", b.identifier("$__t" + i), b.identifier("$__t" + i))
    );
  }
  ...
  inner = inner.concat([
    b.tryStatement(
      b.blockStatement(
        getRestoration(em, isGlobal, localScope, hasTry).concat(finalBody)
      ),
      b.catchClause(
        b.identifier("e"),
        null,
