...
var App = React.createClass({
  ...
  runExpression: function () {
    ...
    function getNodeAtPoint(nodes, cursor) {
      return nodes.reduce(function (acc, topNode) {
        ...
        if (
          (loc.start.line < cursor.line && loc.end.line > cursor.line) ||
          (loc.start.line === cursor.line && loc.start.column <= cursor.ch) ||
          (loc.end.line === cursor.column && loc.end.column >= cursor.ch)
        ) {
          return topNode;
        }
        return acc;
    ...
    function findDeepestFunction(node, cursor) {
      ...
      if (!found) {
        ...
        found.type !== "FunctionDeclaration" &&
    ...
    this.state.client.send({ type: "eval", args: [src] }, function (err, res) {
      console.log(err, res);
    });
  ...
  run: function () {
    ...
    var scripts = resources.state.urls
      .map(function (url) {
    ...
    client.on("ready", function () {
      ...
      client.send({ type: "run", args: [output.code, debugInfo] });
    });

    this.setState({ client: client, debugInfo: debugInfo });
  },
  ...
  render: function () {
    return dom.div(
      ...
      dom.div(
        ...
        Toolset({
          className: this.state.toolsetOpen ? "" : "hidden",
          ref: "toolset",
          toolsetOpen: this.state.toolsetOpen,
          client: this.state.client,
...
var Display = React.createClass({
  ...
  render: function () {
    ...
    return dom.div(
      { className: cls },
      dom.div({ className: "inner" }, dom.iframe())
    );
  },
});
...
var Toolset = React.createClass({
  ...
  render: function () {
    ...
    return dom.div(
      { className: cls },
      dom.button({ className: "close", onClick: this.props.onClose }, "x"),
      dom.ul(
        { className: "nav nav-tabs" },
        dom.li(
          { className: activeClass("debugger", "active") },
          dom.a({ onClick: this.tool.bind(this, "debugger") }, "DEBOOGER")
        ),
        dom.li(
          { className: activeClass("console", "active") },
          dom.a({ onClick: this.tool.bind(this, "console") }, "CONSOLE")
        ),
        dom.li(
          { className: activeClass("resources", "active") },
          dom.a({ onClick: this.tool.bind(this, "resources") }, "RESOURCES")
        )
      ),
      Debugger({
        ref: "debugger",
...
var Debugger = React.createClass({
  getInitialState: function () {
    return { output: "", stack: [], scope: [], runstate: "idle" };
  },
            ...
            stack: stack.reduce(function (acc, frame) {
              if (frame.name != "__global") {
                ...
                acc.push(
                  frame.name +
                    ": " +
                    line.slice(frame.loc.start.column, frame.loc.end.column)
                );
                return acc;
              }
              return acc;
    ...
    if (this.state.runstate == "suspended") {
      childs = childs.concat([
        dom.div(
          ...
          dom.button(
            { className: "btn btn-primary", onClick: this.continue },
            "PLAY"
          ),
          dom.button(
            { className: "btn btn-primary", onClick: this.step },
            "STEP"
          ),
          dom.button(
            { className: "btn btn-primary", onClick: this.stepOver },
            "STEP OVER"
          )
        ),
        dom.div(
      ...
      childs.push(dom.div({ className: "disabled" }, "program not running"));
    }
...
var Resources = React.createClass({
  ...
  render: function () {
    ...
    return dom.div(
      ...
      dom.form(
        { onSubmit: this.add },
        dom.input({ onChange: this.handleInput, value: this.state.inputUrl })
      )
    );
  },
});
...
var Footer = React.createClass({
  render: function () {
    ...
    return dom.div(
      { className: cls },
      dom.button(
        { className: "btn btn-success", onClick: this.props.onRun },
        "RUN"
      ),
      dom.button(
        { className: "btn btn-success", onClick: this.props.onRunExpression },
        "RUN EXPRESSION"
      ),
      dom.button(
        { className: "btn btn-success", onClick: this.props.onToggleToolset },
        this.props.toolsetOpen ? "CLOSE TOOLSET" : "OPEN TOOLSET"
      )
    );
  },
});
