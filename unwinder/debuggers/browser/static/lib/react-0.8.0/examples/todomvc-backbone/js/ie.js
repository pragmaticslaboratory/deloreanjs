...
(function (definition) {
  ...
  else if (typeof YUI == "function") YUI.add("es5", definition);
  else definition();
})(function () {
  ...
  if (!Array.prototype.reduce)
    Array.prototype.reduce = function reduce(fun) {
      ...
      for (; i < length; i++)
        if (i in self) result = fun.call(void 0, result, self[i], i, object);
      return result;
    };
      ...
      Date.parse = function parse(string) {
        ...
        if (match) {
          var year = Number(match[1]),
            ...
            offset =
              !match[4] || match[8] ? 0 : Number(new NativeDate(1970, 0)),
