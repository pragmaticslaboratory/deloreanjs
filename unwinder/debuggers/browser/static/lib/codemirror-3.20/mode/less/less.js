...
CodeMirror.defineMode("less", function (config) {
  ...
  function tokenBase(stream, state) {
    ...
    if (ch == "@") {
      ...
      if (stream.peek() === ")" || type === ":") return ret("number", "unit");
      //rgba(0,0,0,.25);
      ...
      if (stream.current().length == 4 || stream.current().length == 7) {
        if (
          ...
          if (/[\/<>.(){!$%^&*_\-\\?=+\|#'~`]/.test(stream.peek())) {
            if (type === "select-op") return ret("number", "unit");
            else return ret("atom", "tag");
          }
          //#time { color: #aaa }
      ...
      if (stream.current().match(/\\/) !== null) {
        ...
        else if (type === "unit" && state.stack[state.stack.length - 1] === ";")
          return ret(null, "unit");
        else if (type === ")" && state.stack[state.stack.length - 1] === "rule")
          return ret(null, "unit");
        else if (
          type &&
          type.match("@") !== null &&
  ...
  return {
    ...
    token: function (stream, state) {
      ...
      else if (stream.peek() === ":" && stream.current().match(/@|#/) === null)
        style = type;
      if (
        type === ";" &&
        (state.stack[state.stack.length - 1] == "font-family" ||
