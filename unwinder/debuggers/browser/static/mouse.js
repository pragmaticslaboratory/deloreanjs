...
window.onmousemove = function (e) {
  ...
  var mouse = [e.pageX / renderer.scale, e.pageY / renderer.scale];
  ...
  for (var i = 0; i < entities.length; i++) {
    ...
    if (dist < mouseInfluenceSize) {
      ...
      entities[i].lastPos[0] =
        entities[i].pos[0] - (x - lastMouse[0]) * mouseInfluenceScalar;

      entities[i].lastPos[1] =
        entities[i].pos[1] - (y - lastMouse[1]) * mouseInfluenceScalar;
    }
  }
