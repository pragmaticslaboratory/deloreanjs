...
La industria del software se mueve fuertemente hacia el desarrollo de aplicaciones Web, siendo testigo un gran n\'umero de migraciones de aplicaciones {\em standalone} a la Web; los ejemplos desde convertir un documento Word a PDF hasta un sistema ERP ({\em Enterprise Resource Planning})~\pl{CITAS}. Para construir estas aplicaciones, uno de los lenguajes m\'as usados en \javascript, cuya presencia en el entorno de la Web es alrededor de XXX\%~\pl{CITA}. Por ello, cada vez estas aplicaciones se han vuelto más complejas y con mayor riesgo de introducir defectos de software (\aka {\em bugs})~\pl{CITA}. 

	Para detectar y reparar bugs, un conjunto de debuggers se han propuestos, desde el ofrecido por el mismo navegador (\eg {\em developer tools} en Mozilla Firefox) hasta los XXXX~\pl{CITAS}. Lamentablemente, la gran parte de estos debuggers solo permiten mostrar la ocurrencia de un bug sin entregar la posibilidad de retroceder {\em en el tiempo} para tal vez repararlo mientras se ejecuta el programa o manipular el estado de un programa alrededor del bug con el fin de mejorar su comprensi\'on.      
